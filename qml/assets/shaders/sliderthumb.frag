...
layout(std140, binding = 0) uniform buf {
    ...
    float transition; // 0.0 = --slider-thumb-shadow | 1.0 = --slider-thumb-shadow-hover
};
...
void main() {
    ...
    // inset 0px 0px 0px 1px rgba(0, 0, 0, 0.35)
    {
        ...
        float aa = 0.5;
        insetResult = over(insetResult, vec4(vec3(0.0), 0.35 * smoothstep(-aa, aa, d)));
    }
    ...
    // inset 0px 1px 0px 1px rgba(255, 255, 255, 0.3)
    {
        ...
        float aa = 0.5;
        insetResult = over(insetResult, vec4(vec3(1.0), 0.3 * smoothstep(-aa, aa, d)));
    }
    ...
    // inset 0px -1px 0px 1px rgba(0, 0, 0, 0.29)
    {
        ...
        float d = sdRoundBox(p - vec2(0.0, offsetY), boxHalfSize - vec2(spread), r);
        float aa = 0.5;
        insetResult = over(insetResult, vec4(vec3(0.0), 0.29 * smoothstep(-aa, aa, d)));
    }

    // 0px 1px 2px 0px rgba(0, 0, 0, 0.5)  (outer, always present)
    {
        float spread = 0.0;
        float blur = 2.0;
        float offsetY = 1.0;
        vec4 r = max(radii + spread, 0.0);
        float d = sdRoundBox(p - vec2(0.0, offsetY), boxHalfSize + vec2(spread), r);
        outerResult = over(outerResult, vec4(vec3(0.0), 0.5 * (1.0 - smoothstep(-blur, blur, d))));
    }

    // 0px 0px 0px 6px rgba(255, 255, 255, 0.08)  (hover-only ring, driven by `transition`)
    {
        float spread = mix(0.0, 6.0, transition);
        float alpha = mix(0.0, 0.08, transition);
        vec4 r = max(radii + spread, 0.0);
        float d = sdRoundBox(p, boxHalfSize + vec2(spread), r);
        float aa = 0.5;

...
void main() {
    ...
    // inset 0px 0px 0px 1px rgba(0, 0, 0, 0.08)
    {
        ...
        float aa = 0.5;
        insetResult = over(insetResult, vec4(vec3(0.0), 0.08 * smoothstep(-aa, aa, d)));
    }

    // 0px 0px 0px 1px rgba(255, 255, 255, 0.08)  (outer)
    {
        float spread = 1.0;
        vec4 r = max(radii + spread, 0.0);
        float d = sdRoundBox(p, boxHalfSize + vec2(spread), r);
        float aa = 0.5;
        outerResult = over(outerResult, vec4(vec3(1.0), 0.08 * (1.0 - smoothstep(-aa, aa, d))));
    }
